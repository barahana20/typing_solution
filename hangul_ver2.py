        ...
        if(const_value.count('log _{')==1):
            ...
            while(1): # value에 'log _{'가 없어질 때까지 반복
                ...
                if value[value_index+len('log _{'):value.index('}')].find('`')==-1:
                    select_error(hwp, key, const_value, f"'log 밑'(위치: {value_index}번 째 log) 앞 에 약한 공백을 넣어주세요.")
                ...
                if(len(value)-1==value.index('}')):
                    ...
                    select_error(hwp, key, const_value, f"'log 지수'(위치: {value_index}번 째 log) 앞 에 약한 공백을 넣어주세요.")
...
def tk_next_error_find():
    ...
    while(1):
        if(next_error_count>=len(eqn_dict)):
            the_end()
            break
        if(error_find(list(eqn_dict.items())[next_error_count][0], list(eqn_dict.items())[next_error_count][1])==False):
            print(count)
...
def tk_before_error_find():
    ...
    global count
    error_save_log.pop()
    next_error_count = error_save_log.pop()[1]
    tk_next_error_find()
...
btn_explain_label = Label(root, text="다음 에러를 보려면 밑의 버튼을 눌러주세요.")
btn_explain_label.grid(row=2, column=2)
